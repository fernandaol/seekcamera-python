...
def main():
    ...
    with SeekCameraManager(SeekCameraIOType.USB) as manager:
        ...
        # Callback function type that is fired every time the 
        #   camera manager receives an event.
        ''' Manager = Structure that manages Seek cameras. It is the interface through which 
        users can set discovery modes, register event callbacks, and access
        cameras. It is created and destroyed by the user.'''

        while True:
            sleep(1.0)

# This set up gives you the option to run (or not run) a 
#   chunk of code when imported from another python file
# Check if code is being run or imported:
#       if run: name == main
#       if imported == then it doesn't happen
